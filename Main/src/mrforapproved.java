...
public class mrforapproved implements designs {
	...
	public void forapproved() {
		...
		String forapprovedcol[] = { "ID", "NAME", "RESERVATION DATE", "TYPE OF ROOM", "START TIME", "END TIME",
				"DOWNPAYMENT", "GRAND TOTAL" };
		...
		int[] columnWidths = calculateColumnWidths(forapprovedtbl);

		for (int i = 0; i < forapprovedmodel.getColumnCount(); i++) {
			TableColumn column = forapprovedtbl.getColumnModel().getColumn(i);
					...
					if (result == JOptionPane.YES_OPTION) {
						...
						try {
							...
							try (PreparedStatement updateStatement = connection.prepareStatement(updateQuery)) {
								...
								if (rowsAffected > 0) {
									JOptionPane.showMessageDialog(null, "Reservation approved successfully!", "Success",
									...
									JOptionPane.showMessageDialog(null, "Failed to approve reservation.", "Error",
						...
						JOptionPane.showMessageDialog(null, "Approval process cancelled.", "Cancelled",
								JOptionPane.INFORMATION_MESSAGE);
					}
				} else {
					JOptionPane.showMessageDialog(null, "No row selected. Please select a reservation to accept.",
				...
				if (selectedRow != -1) {
					...
					if (result == JOptionPane.YES_OPTION) {
						...
						String reason = JOptionPane.showInputDialog(null,
								"Enter the reason for declining the reservation:");
											...
											if (rowsAffected > 0) {
												...
												try (PreparedStatement deleteStatement = connection
													...
													JOptionPane.showMessageDialog(null,
															"Reservation declined successfully!", "Success",
															JOptionPane.INFORMATION_MESSAGE);
													forapproved.setVisible(false);
													mrforapproved approve = new mrforapproved();
													approve.forapproved();
												...
												JOptionPane.showMessageDialog(null, "Failed to decline reservation.",
					...
					JOptionPane.showMessageDialog(null, "No row selected. Please select a reservation to decline.",
							"No Row Selected", JOptionPane.WARNING_MESSAGE);
				}
			}
		});
