...
public class mraddreservation implements designs {
	...
	// start time and end time combo box

	public void displayEquipmentList() {
		// Initialize the equipmentListModel with fetched data
		equipmentListModel = fetchEquipmentData();

		// Create JList with checkboxes and enable multiple selection
		equipmentList = new JList<>(equipmentListModel);
		equipmentList.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
		equipmentList.setCellRenderer(new CheckBoxListCellRenderer());

		// Add a ListSelectionListener to handle item selection events
		equipmentList.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				// Update the checkbox states based on the selected items
				int[] selectedIndices = equipmentList.getSelectedIndices();
				equipmentList.repaint();
			}
		});

		JScrollPane scrollPane = new JScrollPane(equipmentList);
		scrollPane.setBounds(570, 120, 285, 300);
		addreservation.add(scrollPane);
	}

	private JList<String> equipmentList;
	private DefaultListModel<String> equipmentListModel;

	private DefaultListModel<String> fetchEquipmentData() {
		...
		try {
			...
			while (resultSet.next()) {
				...
				model.addElement(equipmentName);
			}

			connection.close();
		} catch (Exception ex) {
			ex.printStackTrace();
			JOptionPane.showMessageDialog(addreservation, "An error occurred: " + ex.getMessage(), "Error",
	...
	// Helper method to generate time options from 10 AM to 6 PM
	private static String[] generateTimeOptions(String[] hours, String[] ampm) {
		int validStartHour = 10; // Set the minimum valid start hour (inclusive)
		int validEndHour = 18; // Set the maximum valid end hour (inclusive)
		String[] timeOptions = new String[(validEndHour - validStartHour + 1) * ampm.length];
		int index = 0;
		for (int i = validStartHour; i <= validEndHour; i++) {
			// Skip hours from 7 PM to 9 AM
			if (i >= 19 || i < 10) {
				continue;
			}
			for (String period : ampm) {
				timeOptions[index++] = String.format("%02d %s", i % 12, period);
			}
		}
		return timeOptions;
	}

	// Helper method to extract hour from the time option
	private static int extractHour(String timeOption) {
		// Split the time option to extract the hour part
		String[] parts = timeOption.split("\\s+");
		return Integer.parseInt(parts[0]);
	}

	// Helper method to extract period from the time option
	private static String extractPeriod(String timeOption) {
		// Split the time option to extract the period part
		String[] parts = timeOption.split("\\s+");
		return parts[1];
	}
	...
	private class CheckBoxListCellRenderer extends JCheckBox implements ListCellRenderer<Object> {
		public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
			...
			setFont(listbtn); // Replace "Arial" with your desired font

			// Adjust background color based on selection
			setBackground(isSelected ? list.getSelectionBackground() : list.getBackground());
			setForeground(isSelected ? list.getSelectionForeground() : list.getForeground());

			// Check if the item is selected and set the checkbox accordingly
	...
	public void addreservations() {
		...
		String[] hours = { "10", "11", "12", "01", "02", "03", "04", "05", "06", "07", "08", "09" };
		String[] ampm = { "AM", "PM" };

		String[] timeOptions = generateTimeOptions(hours, ampm);
		Arrays.sort(timeOptions,
				Comparator.comparing(mraddreservation::extractPeriod).thenComparingInt(mraddreservation::extractHour));
		// Helper method to extract hour from the time option

		JComboBox<String> startTimeComboBox = new JComboBox<>(timeOptions);
		startTimeComboBox.setSelectedItem("10 AM"); // Default start time
		startTimeComboBox.setFont(subtitle);

		JComboBox<String> endTimeComboBox = new JComboBox<>(timeOptions);
		endTimeComboBox.setSelectedItem("06 PM"); // Default end time
		endTimeComboBox.setFont(subtitle);
		...
		String roomtypearr[] = { "MUSIC ROOM 1", "MUSIC ROOM 2", "MUSIC ROOM 3", "RECORDING ROOM 1", "RECORDING ROOM 2",
				"DELUXE ROOM 1", "DELUXE ROOM 2" };
		...
		contactnumtf.addKeyListener(new KeyAdapter() {
			public void keyTyped(KeyEvent e) {
				...
				if (!Character.isDigit(inputChar)) {
					...
					JOptionPane.showMessageDialog(null, "Please enter number only.", "Invalid Input",
							JOptionPane.ERROR_MESSAGE);
				}
			}
		});
		...
		compute.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (fullname.isEmpty() || address.isEmpty() || contactnum.isEmpty() || datereservation.isEmpty()
							|| starttime.isEmpty() || endtime.isEmpty() || addonpax.isEmpty()) {
						JOptionPane.showMessageDialog(null, "Please fill in all required fields.");
						return;
					}
					if (!isValidDateFormat(datereservation) || !isValidTimeFormat(starttime)
					...
					// ETO MAGIGING CATCHER NG RATES NILA
					// for (Object selectedEquipment : equipmentList.getSelectedValuesList()) {
					// String equipmentName = selectedEquipment.toString();
					// // Add the cost of the selected equipment to addontotal
					// // You might need to fetch the cost of each equipment from the database
					// // and add it to addontotal.
					// // Example: addontotal += getEquipmentCost(equipmentName);
					// }
					...
					JOptionPane.showMessageDialog(null,
							"Invalid value for Additional Pax. Please enter a valid number.");
				} catch (Exception ex) {
		...
		addreservation.add(startTimeComboBox);
		addreservation.add(endTimeComboBox);
		...
		startTimeComboBox.setBounds(300, 101, 240, 30);
		endTimeComboBox.setBounds(300, 164, 240, 30);
		starttime.setBounds(300, 74, 240, 30);
		// starttimetf.setBounds(300, 101, 240, 30);
		endtime.setBounds(300, 137, 240, 30);
			...
			try {
				...
				try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
					...
					if (resultSet.next()) {
						...
						JOptionPane.showMessageDialog(null,
								"There is an overlapping reservation. Please choose a different time.");
						return true;
					}
				}
				connection.close();
